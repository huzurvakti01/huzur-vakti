...
class _SettingsScreenState extends State<SettingsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ErrorPresenter.showSnackBar(
          context,
          error,
          fallback: AppStrings.adhanDownloadFailed,
        );
      }
    } finally {
  ...
  Future<void> _autoSelectCalculationMethod() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${AppStrings.localizationAutoMethod}: ${method.title}'),
        ),
      );
    } catch (error) {
      if (mounted) ErrorPresenter.showSnackBar(context, error);
        ...
        children: [
          ...
          GlassCard(
            child: ListTile(
              ...
              title: const Text(
                AppStrings.openLanguageCountrySetup,
                style: TextStyle(fontWeight: FontWeight.w900),
              ),
              subtitle: Text(
                '${globalSettings.country.flag} '
                '${globalSettings.country.label(context.locale.languageCode)} '
                '• ${globalSettings.method.title}',
              ),
          ...
          GlassCard(
            child: ListTile(
              ...
              title: const Text(
                AppStrings.localizationConsent,
                style: TextStyle(fontWeight: FontWeight.w900),
              ),
          ...
          GlassCard(
            child: ListTile(
              leading: Icon(
                auth.isSignedIn ? Icons.verified_user_rounded : Icons.person_outline_rounded,
              ),
              title: Text(
              ...
              subtitle: Text(
                auth.isSignedIn ? AppStrings.cloudSyncFreeActive : AppStrings.cloudSyncNeedsLogin,
              ),
              ...
              children: [
                ...
                SegmentedButton<HardWakeChallengeMode>(
                  ...
                  onSelectionChanged: (values) {
                    context.read<HardWakeService>().setMode(values.first);
                  },
                ),
                const SizedBox(height: 10),
                SwitchListTile(
                  ...
                  (s) => FutureBuilder<bool>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return RadioListTile<String>(
                        ...
                        subtitle: Text(
                          downloaded ? AppStrings.adhanUseDownloaded : s.subtitle,
                        ),
                        secondary: Wrap(
                          spacing: 4,
                          children: [
            ...
            child: Column(
              children: [
                _SettingsTile(
                  ...
                  onTap: () => _open(context, '/ai', 'ai'),
                ),
                _SettingsTile(
                  title: AppStrings.quickQaza,
                  icon: Icons.emoji_events_rounded,
                  onTap: () => _open(context, '/gamification', 'gamification'),
                ),
                _SettingsTile(
                  title: AppStrings.travelerTitle,
                  icon: Icons.luggage_rounded,
                  onTap: () => _open(context, '/traveler', 'traveler_settings'),
                ),
                _SettingsTile(
                  title: AppStrings.tahajjudTitle,
                  icon: Icons.dark_mode_rounded,
                  onTap: () => _open(context, '/sleep', 'tahajjud_sleep'),
                ),
                _SettingsTile(
                  title: AppStrings.zakatTitle,
                  icon: Icons.calculate_rounded,
                  onTap: () => _open(context, '/zakat', 'zakat'),
                ),
                _SettingsTile(
                  title: AppStrings.profileBackupTitle,
                  icon: Icons.cloud_sync_rounded,
                  onTap: () => _open(context, '/profile-backup', 'profile_backup'),
                ),
                _SettingsTile(
                  title: AppStrings.appIconTitle,
                  icon: Icons.app_settings_alt_rounded,
                  onTap: () => _open(context, '/app-icon', 'app_icon'),
                ),
                _SettingsTile(
                  title: AppStrings.secureNotesTitle,
                  icon: Icons.lock_rounded,
                  onTap: () => _open(context, '/secure-notes', 'secure_notes'),
                ),
                _SettingsTile(
                  title: AppStrings.premiumLibraryTitle,
                  icon: Icons.headphones_rounded,
                  onTap: () => _open(context, '/premium-library', 'premium_audio_library'),
                ),
                _SettingsTile(
                  title: AppStrings.premiumTitle,
                  icon: Icons.workspace_premium_rounded,
                  onTap: () => _open(context, '/premium', 'premium'),
                ),
              ],
            ),
          ),
          ...
          GlassCard(
            child: Text(
              premium.isPremium ? AppStrings.premiumActiveNoAds : AppStrings.freeAdsPolicy,
            ),
          ),
